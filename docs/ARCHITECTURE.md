...
同时，当前版本还强调一件事：配置入口尽量统一。开发、本地测试和 Jenkins 最终都围绕 `.env` 工作，只是 `.env` 的来源不同。
...
- `docker-compose.yml`：提供 Redis 与 MySQL 依赖
- `Jenkinsfile`：负责 `.env` 准备、依赖安装、后端启动、烟测、pytest、JMeter
...
- 用户管理支持单删和批量清理测试账号
- 关键认证与测试规则文档化
- README、部署说明、Jenkins 和 env 约定保持一致
